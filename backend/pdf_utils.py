...
def process_double(file_path: str, start_page: int = 1, end_page: int = 0, reverse_order: bool = False) -> Tuple[str, str]:
    ...
    if reverse_order:
        # Swap: Even (Backs) -> 1, Odd (Fronts) -> 2
        even_doc.save(path1)
        odd_doc.save(path2)
    else:
        # Standard: Odd (Fronts) -> 1, Even (Backs) -> 2
        odd_doc.save(path1)
        even_doc.save(path2)
...
def process_four(file_path: str, start_page: int = 1, end_page: int = 0, reverse_order: bool = False) -> Tuple[str, str]:
    ...
    # Extract relevant pages first
    temp_doc = fitz.open()
    temp_doc.insert_pdf(src_doc, from_page=start_page-1, to_page=end-1)
    
    # Destination dimensions (A4 Landscape)
    ...
    # Split into front/back for printing
    front_doc = fitz.open()
    back_doc = fitz.open()
    ...
    if reverse_order:
        back_doc.save(path1)
        front_doc.save(path2)
    else:
        front_doc.save(path1)
        back_doc.save(path2)
...
def process_book(file_path: str, start_page: int = 1, end_page: int = 0, reverse_order: bool = False) -> Tuple[str, str]:
    src_doc = fitz.open(file_path)
    total_pages = len(src_doc)
...
def encrypt_pdf(file_path: str, password: str) -> str:
    ...
    doc.save(output_name, encryption=fitz.PDF_ENCRYPT_AES_256, owner_pw=password, user_pw=password)
    doc.close()
